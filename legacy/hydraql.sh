...
THREADS=6
OUTPUT_FORMAT="csv"
# Default query directory; will be overridden if --query-dir is used
QUERY_DIRS=("$HOME/Git/codeql")
HAS_USER_QUERY_DIR=false
FANCY=false
DRY_RUN=false
SEVERITY_FILTER=""
INSTALL_PACKS=false
...
Options:
  ...
  --parallel <num>        Alias for --threads
...
Example:
  $0 --db-root ./cqlDB_prj-high5 --langs javascript,typescript --query-dir ~/Git/codeql,~/Git/CQL \
     --output-format sarif --severity HIGH --fancy --pack-install
EOF
  exit 0
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --query-dir)
      if ! $HAS_USER_QUERY_DIR; then
        QUERY_DIRS=()
        HAS_USER_QUERY_DIR=true
      fi
      shift
      while [[ $# -gt 0 && ! "$1" =~ ^-- ]]; do
        IFS=',' read -ra SPLIT <<< "$1"
...
for dir in "${QUERY_DIRS[@]}"; do
  for lang in "${FOUND_LANGS[@]}"; do
    if [[ -d "$dir" ]]; then
      find "$dir" -type f \( -name "*.ql" -o -name "*.qls" \) | grep -Ei "/$lang/" >> tmp_query_list.txt
    fi
  done
done
...
run_query() {
  ...
  if [[ ! -d "$db_path" ]]; then
    echo "❌ Skipping $query — no DB for $lang"
    return
  fi
